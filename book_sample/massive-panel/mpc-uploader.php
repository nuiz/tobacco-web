...
if ( ! function_exists( 'mp_mlu_init' ) ) {
	function mp_mlu_init () {
		global $shortname;
		register_post_type( $shortname.'_assets', array(
			'labels' => array(
				'name' => __( 'Massive Panel Internal Container', 'responsive-flipbook' ),
			),
			'public' => true,
			'show_ui' => false,
...
if ( ! function_exists( 'mp_mlu_js' ) ) {
	...
	function mp_mlu_js () {
		...
		wp_register_script( 'mp-medialibrary-uploader', MP_ROOT.'/massive-panel/js/mp-uploader.js', array( 'jquery', 'thickbox' ) );
...
if ( ! function_exists( 'mp_medialibrary_uploader' ) ) {
	...
	function mp_medialibrary_uploader( $_id, $_class, $_token, $_book_id, $_page_id, $_value, $_mode = 'full', $_desc = '', $_help_desc, $_postid = 0, $_name = '') {
		global $shortname;
		...
		if ( $_desc != '' ) {
			...
			if($id == 'rfbwp_fb_page_bg_image' || $id == 'rfbwp_fb_page_bg_image_zoom') {
				$output .= '<div class="help-icon"><span class="mp-tooltip"><span class="arrow"></span>'.$_help_desc.'</span></div>';
			}
		}
		...
		if($value == "") { 
			if($custom_class == 'rfbwp-page-bg-image') {
				$output .= '<img src="'.MP_ROOT.'/massive-panel/images/no-image.png" />';
			} elseif ($custom_class == 'rfbwp-page-bg-image-zoom') {
				$output .= '<img src="'.MP_ROOT.'/massive-panel/images/no-zoom-image.png" />';
			}
		} else {
			if($custom_class == 'rfbwp-page-bg-image') {
				$output .= '<img src="'.MP_ROOT.'/massive-panel/images/no-image.png" style="display:none;"/>';
			} elseif ($custom_class == 'rfbwp-page-bg-image-zoom') {
				$output .= '<img src="'.MP_ROOT.'/massive-panel/images/no-zoom-image.png" style="display:none;"/>';
			}
		}
		...
		$output .= '<a id="upload_' . $id . '" class="upload_button mp-grey-button '.$custom_class.'" rel="' . $int . '"><span class="left"><span class="icon-upload"></span><span class="desc">Upload Image</span></span><span class="right"></span><span class="left-hover"><span class="icon-upload"></span><span class="desc">Upload Image</span></span><span class="right-hover"></span></a>';
...
if ( ! function_exists( 'mp_mlu_get_silentpost' ) ) {
	...
	function mp_mlu_get_silentpost ( $_token ) {
		...
		if ( $_token ) {
			...
			$_args = array( 'post_type' => $shortname.'_assets', 'post_name' => 'of-' . $_token, 'post_status' => 'draft', 'comment_status' => 'closed', 'ping_status' => 'closed' );
