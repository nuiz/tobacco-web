...
(function () {
	...
	function resizeFB(fbWidth, fbHeight, flipbook, fbCont, zoomed) {
		...
		if(is_IE && !showArrows)
			fbCont.find('div.preview, div.next').css('top', flipbook.find('div.turn-page-wrapper.first').height() - 96);
		else
	...
	function centerBook(page, flipbook, fbCont, activeArrow) {
		...
		if(is_IE && !showArrows) {
			nextY = left + width - 96;
			prevY = left + 10;
			nextYend = left + width - 10;
			prevYend = left - 86;
		} else {
			nextY = left + width - 40;
			prevY = left - 45;
			nextYend = left + width - 50;
			prevYend = left;
		}
	...
	function fbFirstRun(flipbook, fbCont) {
		...
		// IE8
		if(is_IE) {
			jQuery('.page-transition').parent().addClass('page-transitions');
	...
	jQuery(document).ready(function($) {
		var flipbook = $('.flipbook'),
			...
			fbZoom = fbCont.find('div.fb-zoom-out'),
		...
		flipbook.find('div.turn-page-wrapper').addSwipeEvents().on('doubletap', function(e, touch) {
		...
		flipbook.find('div.turn-page-wrapper').dblclick(function(e) {
			...
			zoomCont = $(this).find('div.page-content').clone(true).addClass('zoomed');
			...
			fbZoom.css('visibility', 'visible');
			fbZoom.stop(true).animate({'opacity': 1}, 300);
			...
			function closeZoom() {
				...
				fbZoom.animate({'opacity': 0}, 300, function() {
					fbZoom.css('visibility', 'hidden');
				});
			...
			zoomCont.mousemove(function(evt){
				if(fbTopMargin - evt.pageY >  - zoomCont.children().height() + zoomCont.height()) {
					zoomCont.find(' > div').css('margin-top', (fbTopMargin - evt.pageY) * (zoomFactor - 1));
				} else {
					zoomCont.find(' > div').css('margin-top', - zoomCont.children().height() + zoomCont.height());
				}
	 		});
		...
		if(showArrows || is_IE)
			fbCont.append('<div class="next round' + (!showArrows ? ' hidden' : '') + '"><span class="button"></span><span class="button-over"></span><span class="button-icon"></span></div><div class="preview round' + (!showArrows ? ' hidden' : '') + '"><span class="button"></span><span class="button-over"></span><span class="button-icon"></span></div>');
		...
		$('.fb-nav ul li, .flipbook-container div.next, .flipbook-container div.preview').hover(function(){
			...
			$this.children('span.button-over').stop(true, true).animate( { opacity : 1 }, 300);
		}, function() {
			var	$this = $(this);
			if($this.hasClass('slideshow') && slideshow || $this.hasClass('zoom') && zoomed) {
				//do nothing
			} else {
				$this.children('span.button-over').stop(true, true).animate( { opacity : 0 }, 300);
			}
		});
		...
		$window.on('resize', function() {
			...
			var fbNext = fbCont.find('div.next'),
				...
				ratio, fbWidth, fbHeight, fbParentHeight, fbPercentage,
				navHeight = parseInt(fbNav.css('margin-top')) + parseInt(fbNav.css('margin-bottom')) + parseInt(fbNav.css('top')) + fbNav.height() + 40,
				areaHeight = parseInt(flipbook.css('margin-top')) + parseInt(flipbook.css('margin-bottom')) + flipbook.height() + navHeight,
				position = flipbook.position();

			fbWidth = fbCont.width() - (showArrows ? 92 : 0);
			fbHeight = fbCont.height();
			fbParentHeight = fbCont.parent().height();

			if(fbWidth % 2 == 1)
				fbWidth -= 1;

			ratio = fbWidth / firstWidth;
			...
			if(fbHeight > fbParentHeight) {
				...
				fbWidth = (fbParentHeight) * ratio;
				fbHeight = fbParentHeight;
			}

			if(fbCont.parent().is('#rfbwp_fullscreen') && fbHeight < fbFullscreen.height())
				flipbook.css('margin-top', (fbFullscreen.height() - fbHeight) * 0.5 );
			else
			...
			/* Preview / Next Arrows */
			if(is_IE && !showArrows) {
				fbNext.addClass('small').css({ left: position.left + flipbook.width() - 86 });
				fbPrev.addClass('small').css({ left: position.left - 86 });
			} else {
			...
			if(fbParent.width() < firstNavWidth) {
				fbNav.addClass('slim');

				var buttons = fbNav.find('ul li');

				if(buttons.hasClass('round')) {
					buttons.addClass('was-round center');
					buttons.removeClass('round');
					buttons.first().addClass('left').removeClass('center');
					buttons.last().addClass('right').removeClass('center');
				}

				fbNav.find('ul > li.toc > span.button-icon').css({ left: 13 });
				fbNav.find('ul > li.zoom > span.button-icon').css({ left: 5 });
				fbNav.find('ul > li.slideshow > span.button-icon, ul > li.slideshow > span.button-icon-over').css({ left: 6 });
				fbNav.find('ul > li.show-all > span.button-icon').css({ left: 6 });
				fbNav.find('ul > li.fullscreen > span.button-icon').css({ left: 7 });

				fbNav.width(navWidth);
				fbNav.width(buttons.length * 50 + 12);
			} else {
				fbNav.removeClass('slim');

				var buttons = fbNav.find('ul li');

				if(buttons.hasClass('was-round'))
					buttons.removeClass('was-round center left right').addClass('round');

				buttons.find('span.button-icon, span.button-icon-over').css({ left: '' });

				fbNav.width(firstNavWidth);
			}

			/* Bottom UI */
			var navGap = -40 - parseInt(flipbook.css('margin-bottom')) - parseInt(fbNav.css('margin-top'));

			fbNav.addClass('small').css({'top': navGap, 'margin-bottom': navGap});
			...
			fbNav.find('ul > li.fullscreen > span.button-icon').css({ top: 38 });
